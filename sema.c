...
static int sem_delete(const char *sem_name){
    ...
    if (sp != NULL){
        ...
        else if (sem_destroy(sp) == 0 && sem_unlink(sem_name) == 0)
            rv = SEMA_SUCCESS;
...
int main(int argc, char* argv[])
{
    ...
    if (action_count > 1){
        ...
        if (action != NONE && sem_name != NULL && strlen(sem_name) > 0){
                switch(action){
                        case CREATE:
                            if (argv[optind]){
                ...
                printf("\nSYNOPSIS\n sema -c|-d|-r|-w name [initial value]\n\nDESCRIPTION\n"
                       ...
                       " -w name\n  Wait on the given semaphore. Will block if semaphore value < 0\n"
		);
